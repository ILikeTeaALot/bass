...
mod tests {
	...
	impl TestStruct {
		...
		pub fn handle_sync(self: &mut Arc<Self>, _sync: HSYNC, _channel: DWORD, _data: DWORD) {
	...
	fn test() -> Result<(), Box<dyn Error>> {
		// Put in a block to ensure drop code is run...
		{
			...
			let sync_handler = |user: &mut Sender<i32>, _sync: HSYNC, _channel: DWORD, data: DWORD| {
			...
			let dsp_pan = move |_: &mut Option<u8>, data: &mut [f32], _dsp: HDSP, _channel: DWORD| {
			...
			// Keep a reference to BassDsp to prevent the sync being removed.
			#[allow(unused)]
